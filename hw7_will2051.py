...
def stochastic_hillclimb(num_variables, clauses):
    def randAssignment(num_variables):
        return np.array([-1]*num_variables) ** np.random.randint(2,size=num_variables)
    ...
    while total_score != num_clauses:
        ...
        # initial_score = get_score(state, clauses)
        #with concurrent.futures.ProcessPoolExecutor() as executor:
        #    results = [executor.submit(calcFlippedVar, clause_dict, state, i, next_actions) for i in range(1, num_variables)]
        ...
        best_action = int(abs(np.random.normal(loc=0,scale=std_dev,size=1)))
...
def hw7_submission(num_variables, clauses, timeout):  #timeout is provided in case your method wants to know
    return stochastic_hillclimb(num_variables, clauses)
